...
func _initialize() -> void:
    ...
    if navmesh == null or navmesh.get_polygon_count() <= 0:
        ...
        quit(32)
        return
    var telemetry: Dictionary = NAV_PROBE.probe(navmesh, map_data)
    ...
    if telemetry_path.is_empty() or not FileAccess.file_exists(telemetry_path):
        ...
        quit(38)
        return
    print("MAP_FORGE_PHYSICAL_NAVIGATION_OK routes=%d cells=%d telemetry=%s" % [int(telemetry.get("route_count", 0)), int(grid.get("visited_cells", 0)), telemetry_path])
