...
func _enter_tree() -> void:
    ...
    if children.size() > 1 and children[1] is Label:
        ...
        subtitle.text = "World authoring • synchronized physical path queries • traffic telemetry • formation diagnostics"
...
func _install_probe_controls() -> void:
    ...
    _add_button(bar, "Physical Probe", _run_physical_probe, "Query NavigationServer3D after the navigation map has synchronized")
    _add_button(bar, "Export Traffic", _export_physical_probe, "Export synchronized physical path-query traffic telemetry JSON")
...
func _run_physical_probe() -> Dictionary:
    ...
    if navigation_label:
        navigation_label.text = "[b]Physical Navigation Probe[/b]\nWaiting for NavigationServer3D synchronization..."
    var telemetry: Dictionary = await NAV_PROBE.probe(get_tree(), navmesh, current_map)
    if navigation_label:
        navigation_label.text = NAV_PROBE.format_report(telemetry)
    return telemetry

func _export_physical_probe() -> void:
    var telemetry: Dictionary = await _run_physical_probe()
